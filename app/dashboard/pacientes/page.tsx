  ...
  return (
    <div className="p-8">
      ...
      <form onSubmit={handleSearch} className="mb-6 flex gap-2">
        ...
        <button
          type="submit"
          className="px-4 py-2.5 bg-slate-900 hover:bg-slate-700 text-white text-sm font-medium rounded-lg transition"
        >
          Buscar
        </button>
        {query && (
          <button
            ...
            className="px-4 py-2.5 text-sm text-slate-600 hover:text-slate-900 border border-slate-200 rounded-lg transition"
          >
            Limpiar
          </button>
        )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        {isLoading ? (
          ...
          <div className="py-16 text-center">
            <p className="text-red-500 text-sm">Error al cargar pacientes</p>
          </div>
        ) : data?.data.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  ...
                  <th className="text-left text-xs font-semibold text-slate-500 px-6 py-3">DNI</th>
                  <th className="text-left text-xs font-semibold text-slate-500 px-6 py-3">Edad</th>
                  <th className="text-left text-xs font-semibold text-slate-500 px-6 py-3">Teléfono</th>
                  <th className="text-left text-xs font-semibold text-slate-500 px-6 py-3">Obra social</th>
                ...
                {data?.data.map((patient) => (
                  <tr key={patient.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600">
                      {differenceInYears(new Date(), new Date(patient.fechaNacimiento))} años
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600">
                      {patient.telefono ?? '—'}
                    </td>
              ...
              <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={page === 1}
                    className="px-3 py-1.5 text-xs font-medium text-slate-600 border border-slate-200 rounded-md disabled:opacity-40 hover:bg-slate-50 transition"
                  >
                    Anterior
                  </button>
                  <button
